...
bool LogoScene::init()
{
	...
	static auto background = Sprite::create("ninjan.png");
	background->setPosition(screenSize.width / 2, screenSize.height / 2);
	background->setScale(0.6);
	addChild(background);
	static auto logo = Sprite::create("dao.png");
	logo->setPosition(screenSize.width / 2, screenSize.height / 2);
	logo->setScale(0.2);
	addChild(logo);
	auto moveTo = MoveTo::create(1, Vec2(400, 200));
 ...
 //   logo->runAction(spawn);
	//auto moveTo = MoveTo::create(4, Vec2(200, 240));
 ...
 //   logo->setVisible(false);
	...
	auto gotoNext = CallFunc::create([]() {
		Director::getInstance()->replaceScene(sc1::createScene());
	});
